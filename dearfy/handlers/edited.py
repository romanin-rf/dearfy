...
class EditedItemHandler(ItemHandler):
    REFERENCE_METHOD = dpg.add_item_edited_handler

    def __dearfy_handler_init__(self, parent: Tag) -> None:
        with dpg.item_handler_registry() as handler:
            kwargs = get_method_needed(dpg.add_item_edited_handler, **self._config)
            kwargs.pop('parent', None)
            self._config['tag'] = dpg.add_item_edited_handler(**kwargs)
